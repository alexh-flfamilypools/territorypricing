// TerritoryList.jsx — left rail listing all priced areas
window.TerritoryList = function TerritoryList({ territories, selectedId, onSelect, onAdd, onDelete, query }) {
  ...
  return (
    <aside className="fp-list">
      ...
      <div className="fp-list-body">
        ...
        {filtered.map(t => {
          ...
          return (
            ...
            >
              <div className="fp-list-item-bar" style={{ background: t.color }}></div>
              <div className="fp-list-item-body">
                <div className="fp-list-item-name">{t.name || 'Untitled area'}</div>
                <div className="fp-list-item-price">
                  {t.price ? '$' + Number(t.price).toLocaleString() : <span style={{color:'var(--fg-4)'}}>No price set</span>}
                </div>
              </div>
              <button
                className="fp-list-item-del"
                onClick={(e) => { e.stopPropagation(); if (confirm(`Delete "${t.name || 'this area'}"?`)) onDelete(t.id); }}
                title="Delete"
              >
      ...
      <button className="fp-btn fp-btn-secondary fp-list-add" onClick={onAdd}>
        <i data-lucide="plus"></i> New priced area
      </button>
    </aside>
  );
};
